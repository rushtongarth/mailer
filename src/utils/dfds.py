...
class DataFrameDataset(torchtext.data.Dataset):
    ...
    Create a dataset from a pandas dataframe of
    examples and fields
    ...
    ----------
        ...
        :param fields: Fields to use, where keys are field names, and
                       values are the associated fields.
        :type fields: dict
        :param filter_pred: use only exanples for which
                            filter_pred(example) is true, or use all
                            examples if None.
        :type filter_pred: (callable or None)
    """
    def __init__(self, examples, fields, filter_pred=None):
        self.examples = examples.apply(
            SeriesExample.fromSeries,
